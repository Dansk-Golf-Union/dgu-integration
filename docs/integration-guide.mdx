...
The following query parameters can be used when calling the apis above:

- **changedsince**  
  Format: `yyyymmddThhmmss`. Returns courses changed after a given date.

- **active**  
  Accepts `0` or `1`. Filters based on `Course.IsActive` (0 = inactive, 1 = active).

- **sort**  
  Used to sort the courses based on properties of the `Course` object (see details below).

- **sortTee**  
  Used to sort the individual tees within each course (see details below).

- **excludeParams**  
  Used to exclude properties from the response, e.g. `?excludeParams=tees` to omit `Course.Tees`.

---

#### Sorting with `sort`

Courses can be sorted by adding the `sort` parameter in the following format:

sort=PropName:SortDirection,...

**Valid `PropName` values**:  
...
- Sort by active courses and then newest activation date: `&sort=IsActive:0,ActivationDate:1`

---

#### Sorting with `sortTee`

Tees within each course can be sorted using the `sortTee` parameter in the same format:

sortTee=PropName:SortDirection,...
...
- Sort by gender and then highest rating: `&sortTee=Gender:0,CourseRating:1`
...
| CourseHandicap        | Integer  | No       | The course handicap the round was played off. (Spillehandicap i DK) - if left out PHCP is used              |
| PHCP                  | Integer  | No       | The playing handicap the round was played off (justeret spillehandicap i DK)                 |
| Comment               | String   | No       |                                               |
| Course                | Object   | No       |                                                                                              |
